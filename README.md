...
> 1회차(no1) 브랜치 수동 코드 리뷰 + N=5 자동 측정 데이터 기반. 수동 평가 항목은 주관적 요소 포함.

### 4.1 프론트엔드 (Flutter/Dart) 점수

| 카테고리 | `with` | `without` | 근거 |
|---------|:-:|:-:|------|
| **API 정합성** | **10** | **4** | with: 5/5 통과(100%). without: 3/5(60%), Dart→Python 필드명 불일치 2건 |
| **아키텍처** | 7 | **8.5** | without: Repository 패턴(인터페이스+구현 분리), TypeAdapter, ProcessingStep enum, UrlValidator |
| **에러 처리** | 6 | **7.5** | without: `ApiException` 커스텀 예외(HTTP 상태 코드 포함), `ProcessingStep.error` 상태 머신. with: 일반 `Exception` |
| **UI 완성도** | 7 | **7.5** | without: `ChatWidget` 별도 분리 + 타이핑 애니메이션, 3탭 구성. with: 인라인 채팅, 2탭 |
| **프론트 소계** | **7.5** | **6.9** | API 정합성 가중 50% 적용 |

### 4.2 백엔드 (FastAPI/Python) 점수

| 카테고리 | `with` | `without` | 근거 |
|---------|:-:|:-:|------|
| **설정 관리** | 5 | **8** | without: `config.py` 분리, 환경변수 한 곳 관리. with: `os.getenv()` 산재 |
| **API 설계** | **8** | 6 | with: `TranscriptSegment` 구조체 + `full_text`, 다국어 `language` 파라미터. without: 문자열 반환 |
| **AI/프롬프트** | **8** | 5 | with: temperature 분리(요약 0.3/채팅 0.7), 다국어 프롬프트. without: 한국어 고정, 기본 temperature |
| **외부 연동** | **8** | 5 | with: oEmbed API(안정적). without: HTML 파싱+정규식(YouTube 마크업 변경 취약) |
| **클라이언트 패턴** | 6 | **7** | without: `@lru_cache(maxsize=1)`. with: 글로벌 변수 + `_get_client()` |
| **테스트** | **7** | 5 | with: 15개(채팅 엔드포인트 3건 포함). without: 11개. 특히 채팅 에러 케이스 차이 |
| **백엔드 소계** | **7.0** | **6.0** | 균등 배분 |

### 4.3 공통 메트릭 점수

| 카테고리 | `with` | `without` | 근거 |
|---------|:-:|:-:|------|
| **개발 프로세스** | **9.5** | 3 | with: phase별 커밋, 문서 16개, hook 5개. without: 단일/소수 커밋, 문서 없음 |
| **안정성** | **9** | 6 | with: 타임아웃 0, 시간 편차 작음(379~883초). without: 타임아웃 1, 편차 큼(331~1,807초) |
| **시간 효율** | **8** | 5 | with: 평균 662초. without: 평균 901초 (27% 느림) |
| **비용 효율** | 6 | **7** | without: 평균 $5.8 (17% 저렴). 단, no1 미기록으로 과소평가 가능 |
| **공통 소계** | **8.1** | **5.3** | 균등 배분 |

### 4.4 가중 종합

| 영역 | 가중치 | `with-dev-bounce` | `without-dev-bounce` |
|------|:-----:|:-:|:-:|
| 프론트엔드 | 30% | 2.25 | 2.07 |
| 백엔드 | 20% | 1.40 | 1.20 |
...
> **가중치 설계**: API 정합성(30%)을 별도 분리하여 최대 가중. "앱이 실제로 동작하는가"가 가장 중요하다는 판단. 프론트(30%) > 백엔드(20%)는 사용자 경험 직결 영역 우선.

> **점수 산정 기준**
>
> | 구분 | 측정 방법 | 10점 기준 | 0점 기준 |
> |-----|----------|----------|---------|
> | API 정합성 | `verify-api-contract.py` 자동 검증 (N=5) | 5/5 통과, critical 0 | 0/5 통과 |
> | 프론트 아키텍처 | 수동 리뷰 — 패턴 분리, 타입 안전성, 상태 관리 | 모든 영역 모범 패턴 | 구조 없이 단일 파일 |
> | 백엔드 설계 | 수동 리뷰 — 설정 관리, API 설계, 외부 연동, 테스트 | 각 영역 모범 사례 적용 | 구조 없이 하드코딩 |
> | 개발 프로세스 | 커밋 수, 문서 수, hook/gate 유무 (N=5 공통) | 의미 단위 커밋 + 문서화 + 자동 검증 | 단일 커밋, 문서 없음 |
> | 안정성 | N=5 타임아웃 횟수 + 시간 표준편차 | 0 타임아웃 + 낮은 편차 | 과반 타임아웃 |
> | 시간/비용 효율 | N=5 평균값 비교 | 400초 이하 / $3 이하 | 1,800초 / $20 (상한) |
>
> - **자동 측정**: API 정합성, 안정성, 시간, 비용 — N=5 실험 데이터에서 직접 산출
> - **수동 평가**: 아키텍처, 에러 처리, UI, API 설계 등 — 1회차(no1) 브랜치 코드 리뷰 기준
